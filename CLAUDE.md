...
- **Shop** (stock items) deliberately reuses `TransactionType::Expense`/`OtherIncome` instead of adding new enum cases, so Cash Box/Transactions need zero extra plumbing — a shop transaction is distinguished only by its nullable `shop_item_id`.
